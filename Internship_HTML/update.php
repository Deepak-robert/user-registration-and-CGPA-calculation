...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($password) {
        $stmt = $conn->prepare("UPDATE users SET name=?, bio=?, address=?, email=?, password=? WHERE username=?");
        $stmt->bind_param("ssssss", $name, $bio, $address, $email, $password, $username);
    } else {
        $stmt = $conn->prepare("UPDATE users SET name=?, bio=?, address=?, email=? WHERE username=?");
